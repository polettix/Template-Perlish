...
reflected by option C<utf8> defaulting to I<true> in L<< /B<new> >>.
This default is inhibited by setting C<utf8> to a false value, or
C<binmode> to any defined value.
...
It's also possible to set all options described for constructor L</new>, as long as you

Returns the rendered template, i.e. the same output as L</process>. Note
that it assumes the default values for options explained in L</new>.
...
string to set C<binmode> on the output filehandle. A defined value
disables the default to C<utf8>.

=item I<functions>

(as of version 1.58) hash reference with functions that will be injected
in the C<Template::Perlish> namespace for the duration of the template
evaluation. Keys are assumed to be valid function names, values are
assumed to be valid sub references.

=item I<method_over_keys>

boolean flag used for traversal, see L</traverse>.
...
behaviour was the norm: inside a template, whatever C<print> to
C<STDOUT> is trapped and put in the template expansion. For this reason,
this option defaults to a true value (C<1>) in order to keep backwards
compatibility.

If you set this flag to a false value, C<STDOUT> will not be modified
and will be accessible from within the templates. In this case, if you
still want to I<print> inside the template you can use function L</P>.
...
=item I<strict_blessed>

boolean flag used for traversal, see L</traverse>.

Defaults to I<false>;

=item I<traverse_methods>
...
Defaults to I<true>, unless C<binmode> option is set to a defined value.
...
this is a I<meta>-option and is available only when the constructor is
...
overrides C<stdout> to a I<false> value, while setting
C<traverse_method> and C<method_over_key> to a I<true> one. As a matter
...
times and also use meta-option C<-preset> described above.
...
As of version 1.52, the new boolean option C<stdout> has been
introduced, allowing to keep the old behaviour (i.e. printing to
C<STDOUT> is captured in the expanded template) described above, or to
use the new one where C<STDOUT> is not clobbered. This parameter
defaults to C<1> (i.e. a I<true> value, in Perl sense) for backwards
compatibility. If you still want to I<print> out, though, you can use
the new function L</P>. As a matter of fact, you're encouraged to always
use C<P>, because it will work both in the old and in the new setup.
...
You can look at this accessor as an alternate form for L</traverse>,
only with a slightly different input interface (e.g. defaulting to the
template variables and swapped parameter positions).

=back

=head2 Direct Printing

Up to version 1.50, if you C<print>ed (to C<STDOUT>, which would be the
...
option C<stdout> (see L</new>). By default, its value is such that the
old behaviour is preserved: prints to C<STDOUT> are trapped and put in
the template's expansion.

In case the new option C<stdout> is set to a false value, though, any
C<print> will use the currently selected filehandle before entering the
template, i.e. C<STDOUT> (the I<real> one, not the one set within the
template) by default. This allows your code to actually communicate with
the external world, if you need to.

The following function allows you to still use a C<print>-like interface
...
value of option C<stdout> (see L</new>), this function will always put
its argument inside the template.

In case C<stdout> is I<true> (which is the default value for backwards
compatibility), C<P> and C<print> to C<STDOUT> are equivalent (as a
matter of fact, C<P> just calls C<print> to C<STDOUT> behind the
scenes).

In case C<stdout> is I<false>, instead, C<P> will send its output to the
template, while C<print> will send it to whatever handle is currently
selectd, and C<print> to C<STDOUT> will use the C<STDOUT> available at
...
If C<stdout> is I<true>, it will be expanded to:

   Hey 'foo'
   I spoke with 'bar'
   Do you know 'baz'?

If C<stdout> is I<false>, it will be expanded to:

   Hey ''
   I spoke with ''
   Do you know 'baz'?

while the string C<foo> will be printed to the currently selected
filehandle (which is usually C<STDOUT>, but you might have something
different in your program), and the string C<bar> is printed to
C<STDOUT> as you know it.
...
argument for supporting I<partial matches>. If false (default), the legacy
behaviour is kept, and the provided input C<$path> must match in its
entirety or the splitting will fail. If true, the extraction of crumbs will
begin at the first character of C<$path> but be allowed to stop in the
middle; this e.g. allows parsing strings like C<foo.bar:baz>, with crumbs
resulting in C<< ['foo', 'bar'] >>. The first variant will only return the
...
(or, as of version 1.62, whatever is passed with option C<missing>, if
...
described in L</Variables Accessors>.
...
what is passed as option C<missing> if present) is returned otherwise.
...
Object C<$object> is a blessed hash reference with two keys, two methods
...
=item C<method_over_key>

when traversal by method is enabled (see C<traverse_methods> below),
this option allows controlling whether the key or the method win when
both are present.

In the example above, if this option is set to a I<true> value the path
C<what> triggers a method call; otherwise, the value in the hash is
considered (i.e. C<ever> in the example). When a key is not present, the
method is called as a fallback.

Defaults to a I<false> value.

=item C<strict_blessed>
...
disables I<peeking> inside the object in case a method is not available
but a key exists.

In the example above, using path C<what> always triggers the method call
(returning C<hey>), using path C<urgh> calls the method call, using path
C<foo> returns the empty string.

This option hides option C<method_over_key>.

Defaults to a I<false> value.

=item C<traverse_methods>
...
In the example object, if this parameter is set to a I<false> value the
object will be considered just a plain hash. Otherwise, what a visit for
keys C<what>, C<urgh> and C<foo> returns depends on the options above.
...
Since version 1.62, additional option C<missing> allows setting the
...
empty string, as opposed to C<undef> that is the old behaviour.
...
Copyright (c) 2008-2016 by Flavio Poletti C<polettix@cpan.org>.
