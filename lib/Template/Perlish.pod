...
This document describes Template::Perlish version 1.41_02.
...
will be able to actually I<traverse> only arrays or hashes, so you'd
better provide an empty path if you set C<data> to anything else);

=item C<path>

the path to follow inside C<data>. It can be either a plain string that
will be split using L</crumble>, or an array reference containing the
different I<crumbs> to follow;

=item C<ref>

=item C<reference_to_value>

boolean value to request whether a value or a reference to a value are
needed in output. C<ref> and C<ref_to_value> are synonyms, with the
former taking precendence on the latter if both are present.

=back

Depending on C<ref>, you will get either a value back (if it is false)
or a reference to it (if it is true). This parameter also changes how
the traversal is done in case of missing parts.

In particular, you will want to set C<ref> to a false value if you want
to just I<read> from C<data>. In this case, the first missing crumb will
...
If you set C<ref> to a true value instead, you will get back a reference
...
I<write> into the data structure, like in the following example:
...
returned in case C<ref> is false, otherwise C<undef> will be returned.

If C<path> is not provided, or set to the empty string, C<data> or a
reference to it is returned (depending on C<ref>, see below). This is
the only real case in which C<data> can actually be set to whatever you
want, otherwise you should stick to either hash or array references.

C<path> components can be plain scalars or references themselves. When
they are plain scalars, they will be used directly to access C<data> or
its descendants; otherwise, C<traverse> will enforce that the current
descendant in C<data> is a reference of the same type as the specific
...
As already mentioned, auto-vivification kicks in if C<ref> is true. In
this case, a reference in the C<path> will also force a specific
auto-vivification type, i.e. the automatic creation of either an array
or a hash reference. If the crumb in C<path> is not a reference, a guess
is taken in that non-negative integers are considered indexes of an
array, otherwise a hash is assumed. So, let's take C<$path4> again, and
let's see what happens when C<ref> is true:
